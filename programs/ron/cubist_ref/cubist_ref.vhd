...
begin
    ...
    begin
        if rising_edge(clk) then
            if fr_done = '1' and prev = '0' then
                report "REF ang=" & integer'image(to_integer(an_yaw))
                    & "," & integer'image(to_integer(an_pit))
                    & "," & integer'image(to_integer(an_rol))
                    & " nslot=" & integer'image(to_integer(s_nslot))
                    & " H=" & integer'image(to_integer(t_dh0))
                    & "," & integer'image(to_integer(t_dh1))
                    & "," & integer'image(to_integer(t_dh2));
                for k in 0 to 2 loop
                    report "REF slot" & integer'image(k)
                        ...
                        & " gux=" & integer'image(to_integer(sl_gux(k)))
                        & " guy=" & integer'image(to_integer(sl_guy(k)))
                        & " gvx=" & integer'image(to_integer(sl_gvx(k)))
                        & " gvy=" & integer'image(to_integer(sl_gvy(k)))
                        & " hn=" & integer'image(to_integer(sl_hn(k)))
                        & " hu=" & integer'image(to_integer(sl_hu(k)))
                        & " hv=" & integer'image(to_integer(sl_hv(k)))
                        & " sil=" & integer'image(to_integer(unsigned(sl_sil(k))));
                end loop;
                for k in 0 to 2 loop
                    report "REF gram" & integer'image(k)
                        & " ymin=" & integer'image(to_integer(signed(gram(k*32+0))))
                        & " ymax=" & integer'image(to_integer(signed(gram(k*32+1))))
                        & " e0=" & integer'image(to_integer(signed(gram(k*32+2))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+3))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+4))))
                        & " e1=" & integer'image(to_integer(signed(gram(k*32+5))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+6))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+7))))
                        & " e2=" & integer'image(to_integer(signed(gram(k*32+8))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+9))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+10))))
                        & " e3=" & integer'image(to_integer(signed(gram(k*32+11))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+12))))
                        & "/" & integer'image(to_integer(signed(gram(k*32+13))));
                end loop;
            end if;
            prev := fr_done;
        end if;
